...
namespace PixelReyn.VoxelSeries.Part2
{
    ...
    public class Container : MonoBehaviour
    {
        ...
        public void DamageVoxel(Vector3 hitPoint, Vector3 hitNormal, int damage)
        {
            // 1. Calculate the center of the block specifically INSIDE the mesh
            // We move slightly into the block opposite to the normal to find the integer coordinate
            Vector3 localPos = hitPoint - (hitNormal * 0.5f);
            
            // 2. Round to get the exact grid coordinate (Key)
            Vector3 blockCoord = new Vector3(
            ...
            if (data.ContainsKey(blockCoord))
            {
                Voxel v = data[blockCoord];
                
                // Prevent underflow
                int newHealth = v.Health - damage;

                if (newHealth <= 0)
                {
                    // Block Destroyed: Remove from data
                    data.Remove(blockCoord);
                }
                else
                {
                    // Block Damaged: Update health and put back in struct
                    v.Health = (byte)newHealth;
                    data[blockCoord] = v;
                    
                    // Optional: You could change v.ID here to a "cracked" texture ID if you have one
                }

                // 4. Important: Regenerate the mesh to show changes
                // In a huge world, you would only update the specific chunk. 
                // Since this is one Container, we update the whole thing.
                GenerateMesh();
                UploadMesh();
            }
        }
